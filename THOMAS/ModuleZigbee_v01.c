...
//Initialisation GPIO du module Zigbee
...
//Lecture de l'Usart2 et stockage dans le Buffer
static inline void Zigbee_Receiv_Usart2(char* pBuffer)
{
	int i=0;
...
//Lecture du buffer de reception et séparation des différents elements
inline void Zigbee_Receive_Data( struct zigbeeData * const pxZigbeeData )
{
	int i=0;
	int indexParam;
	
	ZigBeeReceiv_Usart2(Buffer);
	indexParam = Buffer[i++];
...
//Envoi d'une trame vers l'Usart2 du module Zigbee
static inline void Zigbee_Send_Usart2(char Trame[])
{
	int i=0;
...
//Envoi de la trame "Status" vers la base
								...
								const struct droneTelemeterWrapper *const pxCurrTelemeterWrapper, const struct droneBatteryWrapper *const pxCurrBatteryWrapper)
{
	...
	Buffer[i] = NULL;
	Zigbee_Send_Usart2(Buffer);
}

//Permet la verification de configuration avec le module Zigbee
...
//Lance la configuration du module Zigbee
...
//Permet un reset Soft du module Zigbee
...
//Mesure le niveau du signal du dernier message recu
inline int Zigbee_Level(char* pBuffer)
{
	...
	Send_Usart2("ATDB");	//Mesure le niveau du signal
	Receiv_Usart2(pBuffer);
	sSend_Usart2("ATCN");	//Quitte le mode de configuration
